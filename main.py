...
def video_info():
    for i in range(len(videos)):
        ...
        try:
            ...
            videos[i] += [response['items'][0]['statistics']['favoriteCount']]
...
def write_csv():
    with codecs.open('datas.csv', "w", "utf-8") as dataFile:
        dataFile.write(
            "playlist_name;video_id;video_name;publication_date;views;likes;dislikes;favorites;comments;duration" + "\n")
        for i in range(len(videos)):
            if videos[i][2] != "Private video" and videos[i][2] != "Deleted video":
                for j in range(9):
                    dataFile.write(str(videos[i][j]) + ";")
                dataFile.write(str(videos[i][9]) + "\n")
